...
class EnchantItem {
  ...
  getLimit(type: StatNormalTypes): { max: number; min: number } {
    ...
    // const add = Math.max(Math.floor((enchantStates.Character.level - 200) / 10) * 5, 0)
    ...
    return {
      ...
      max: Math.min(max + extraLimitMax, originallvLimit),
    }
  }
  ...
  getPotentialConvertThreshold(type: StatNormalTypes) {
    ...
    return (
      value ||
      Math.min(
        enchantStates.PotentialConvertDefaultThreshold,
