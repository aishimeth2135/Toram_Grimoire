...
function PageInitFirst(config){
    ...
    if ( !config.isHomePage ){
        ...
        nav.innerHTML = `<ul><li><a href="../">${GetLang('Top List/item 1')}</a></li><li class="Cyteria scope-icon">${Icons('arrow-right')}</li><li><span>${GetLang('Top List/item 2')}</span></li></ul>` + nav.innerHTML;
    }

    const loadingPage = document.createElement('div');
    loadingPage.id = 'global--Loading-Page';
    const loadingPage_contanier = simpleCreateHTML('div', 'container');
    loadingPage_contanier.append(
        simpleCreateHTML('div', 'main', Icons('potum')),
        simpleCreateHTML('div', 'msg')
    );
    loadingPage.appendChild(loadingPage_contanier);
    loadingPage.appendChild(simpleCreateHTML('div', 'bottom-tips', GetLang('Loading Page/bottom tips')));
