    ...
    <div v-if="type == 'normal'" class="drag-bar-layout">
        ...
        <div class="content">
             <div class="drag-bar-container">
                ...
                <div class="mask"
                    @mousedown.prevent="startMouseDown"
                    @mousemove.prevent="updateValue($event)"
                    @click.prevent="updateValue($event)"
                    @mouseup.prevent="cancelMouseDown"
                    @mouseleave.prevent="cancelMouseDown">
                </div>
            </div>
...
<script>
    export default {
        ...
        computed: {
            offset(){
                return Math.floor(100 * this.value / (this.range[1] - this.range[0])) + '%';
            }
        },
        methods: {
            cancelMouseDown(){
