...
export function setupPopperOptions(options: PopperOptions = {}) {
  ...
  ;(Object.entries(defaultValueMapping) as ([keyof PopperOptions, any])[]).forEach(([key, value]) => {
    _options[key] = computed(() => options[key] === undefined ? value : options[key])
  })
...
export function initCyPopper() {
  document.body.addEventListener('click', (evt: MouseEvent) => {
    ...
    document.querySelectorAll('#app-popovers > .cy--popper').forEach(el => el.dispatchEvent(customEvent))
  }, { capture: true })
}
