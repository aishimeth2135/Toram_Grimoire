...
export default function () {
  ...
  const cyBracket: (marked.TokenizerExtension & marked.RendererExtension) = {
    ...
    renderer(token) {
      return `<span class="border-l-1 border-r-1 border-current mx-2 px-2 text-light-4">${this.parser.parseInline(token.tokens!)}</span>`
    },
  }
  marked.use({
    renderer: {
      checkbox(checked) {
        ...
        const textClass = checked ? 'text-light-3' : 'text-light-2'
