<template>
  <AppLayoutMain class="py-4">
    <template #column(0)>
      <textarea
        v-model="currentText"
        class="book-preview-textarea m-0 block h-full w-full resize-none border-0 bg-white p-3 !font-mono outline-none"
      />
    </template>
    <template #column(1)>
      <div>
...
watch(
  ...
  value => {
    ...
    if (
      ...
      (parse.errors.length === 1 &&
        parse.errors[0].code === 'UndetectableDelimiter')
    ) {
