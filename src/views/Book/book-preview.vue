<template>
  <AppLayoutMain class="py-4">
    <template #column(0)>
      <textarea
        v-model="currentText"
        class="book-preview-textarea border-0 m-0 outline-none resize-none p-3 block w-full h-full !font-mono bg-white"
      />
    </template>
    <template #column(1)>
      <div>
...
watch(currentText, value => {
  ...
  if (parse.errors.length === 0 || (parse.errors.length === 1 && parse.errors[0].code === 'UndetectableDelimiter')) {
