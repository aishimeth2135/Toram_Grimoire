...
<template>
  <div class="w-full space-y-6">
    <div>
      <CommonPropInput v-model:value="equipment.name"
    ...
    <div v-if="equipment.customTypeList">
      ...
      <cy-button-radio-group v-model:value="equipment.type" :options="equipment.customTypeList.map(item => ({
    ...
    <div v-if="equipment.is(EquipmentKinds.Weapon)" class="flex flex-wrap items-center">
      ...
      <CommonPropNumberInput v-if="equipment.hasStability" v-model:value="equipment.stability"
        :title="$t('character-simulator.equipment-info.stability')" unit="%" range="0~100" />
    </div>
    ...
    <div v-if="equipment.hasRefining">
      <CommonPropNumberInput v-model:value="equipment.refining"
        :title="t('character-simulator.equipment-info.refining')" range="0~15" />
    </div>
