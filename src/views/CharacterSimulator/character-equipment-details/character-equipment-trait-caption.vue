...
const RenderCaption = ({ text }: { text: string }) => {
  const { parts } = handleParseText(text, [getCommonTextParseItem(CommonTextParseItemIds.Value)])
...
const handleStatValue = (valueStr: string) => {
...
<template>
  ...
  <div v-else-if="firstStat">
    <ShowStatComputed :stat="firstStat" :handle-value="handleStatValue" />
  </div>
  <div v-else>
    {{ t('global.none') }}
  </div>
