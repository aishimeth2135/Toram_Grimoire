      ...
      <div class="w-full pr-3">
        ...
        <div class="flex flex-wrap items-center">
          <div class="flex items-center" @click="skillLevelInput?.select()">
            <div class="text-primary-40 mr-1">Lv.</div>
            <div>
              <input
                ref="skillLevelInput"
                v-model="editedSkillLevel"
                type="number"
                class="text-primary-70 w-8 bg-transparent"
              />
            </div>
          </div>
        ...
        <div v-if="skillLevelChanged" class="mt-2 flex items-center">
          <div class="text-gray-40 mr-1">Lv.</div>
          <span class="text-gray-50">{{ skillLevel }}</span>
          <cy-icon icon="ic:round-arrow-forward" class="text-primary-20 mx-1.5" />
          <div class="text-primary-40 mr-1">Lv.</div>
          <span class="text-primary-60">{{ editedSkillLevel }}</span>
        </div>
        <div v-if="skillLevelChanged" class="my-2">
          <div class="mb-4 space-x-3">
            <cy-button-circle icon="mdi:done-outline" small @click="applySkillLevel" />
            <cy-button-circle
              icon="mdi:close"
              color="gray"
              small
          ...
          <div v-if="effectedSkills.length > 0">
            <div class="text-red-60 text-sm">
              {{ t('character-simulator.skill-build.apply-skill-level-tips.0') }}
            </div>
            <div class="text-primary-40 mt-1 text-sm">
              {{ t('character-simulator.skill-build.apply-skill-level-tips.1') }}
            </div>
            <div class="border-primary-10 mt-3 rounded-sm border p-3">
              ...
              >
                <div class="shrink-0 self-stretch pr-3">
                  <div class="border-primary-20 flex rounded-full border-2 bg-white p-1.5">
...
const applySkillLevel = () => {
  props.skillBuild.setSkillLevel(props.skill, editedSkillLevel.value, effectedSkills.value)
...
const incValue = (inc: number) => {
  editedSkillLevel.value = Math.min(Math.max(0, editedSkillLevel.value + inc), 10)
