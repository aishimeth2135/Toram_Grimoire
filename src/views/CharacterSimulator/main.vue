    ...
    <article>
        <div class="main">
            ...
            <section v-else-if="currentContent == 1">
                <div class="character-fields">
                    <div v-for="(field, i) in currentCharacterState.origin.equipmentFields"
                        class="character-field">
                        <div class="top">
                            <div class="field-name">{{ langText('character field names/' + field.type.description) }}</div>
                            <div class="buttons">
                                <cy-button iconify-name="ic-outline-category" type="icon-only" class="single-line"
                                @click="selectFieldEquipment(field)" />
                            </div>
                        </div>
                        <equipment-information v-if="!field.empty()"
                            @open-select-crystal="openSelectCrystal"
                            :equipment-data="getShowEquipmentData(field.equipment)"
                            @click.native.stop />
            ...
            <div class="tail">
                <cy-button iconify-name="mdi-rabbit" class="no-border"
                    @click="switchContent(0)">
                    {{ langText('character') }}
                </cy-button>
                <cy-button iconify-name="mdi:sword" class="no-border"
                    @click="switchContent(1)">
                    {{ langText('equipment') }}
                </cy-button>
                <cy-button iconify-name="fa-solid:star-half-alt" class="no-border"
                    @click="switchContent(2)">
                    {{ langText('skill') }}
                </cy-button>
            </div>
        </div>
        <div class="window-container">
            <cy-window :visible="browseEquipmentsWindowState.visible"
                ...
                class="frozen-top width-wide browse-equipment-window">
                <template v-slot:title>{{ langText('browse equipments/action: ' + browseEquipmentsWindowState.action) }}</template>
                <template v-slot:default>
                    <div class="buttons">
                        <cy-button type="icon-only" iconify-name="ic-round-add-circle-outline"
                            @click="appendEquipmentWindowState.visible = true">
                        </cy-button>
                    </div>
                    <div class="equipments-scope">
                        ...
                        <div v-else class="equipments">
                            <cy-button v-for="(data, i) in browsedEquipmentDatas"
                                type="line" class="inline"
                                :key="data.origin.id"
                                :iconify-name="data.categoryIcon"
                                @click="browsedEquipmentSelect(data)">
                                {{ data.origin.name }}
                            </cy-button>
                        </div>
                        <div class="bottom-menu" v-if="browseEquipmentsWindowState.currentEquipmentData != null">
                            <div class="detail content">
                                <div class="info">
                                    <equipment-information
                                        :equipment-data="browseEquipmentsWindowState.currentEquipmentData"
                                        @open-select-crystal="openSelectCrystal" />
                                </div>
                            ...
                            <div class="tail-buttons" v-if="browseEquipmentsWindowState.action == 'select-field-equipment'">
                                <cy-button iconify-name="ic-round-done"
                                    @click="equipmentConfirmSelection">
            ...
            <cy-window :visible="appendEquipmentWindowState.visible"
                @close-window="appendEquipmentWindowState.action == 'select-mode' ?appendEquipmentWindowState.visible = false : appendEquipmentWindowState.action = 'select-mode'"
                class="frozen-top width-wide">
                <template v-slot:title>{{ langText('append equipment/window title: ' + appendEquipmentWindowState.action) }}</template>
                <template v-slot:default>
                    <transition name="fade" mode="out-in">
                        <div v-if="appendEquipmentWindowState.action == 'select-mode'" key="select-mode">
                            <cy-button iconify-name="mdi:sword" type="description"
                                @click="setAppendEquipmentAction('search')">
                                {{ langText('append equipment/action: search') }}
                                <template v-slot:description>
                                    {{ langText('append equipment/action: search description') }}
                                </template>
                            </cy-button>
                            <cy-button iconify-name="ic-outline-category" type="description"
                                @click="setAppendEquipmentAction('custom')">
                        ...
                        <div v-else-if="appendEquipmentWindowState.action == 'search'" key="search" class="append-equipment--search">
                            <cy-title-input iconify-name="ic-outline-category">
                                <input type="text" ref="appendEquipmentSearchText"
                                    :placeholder="langText('append equipment/search equipment placeholder')"
                                    @keyup.enter="updateAppendEquipmentSearchResult" />
                                <cy-button iconify-name="bx-bx-search-alt-2"
                                    @click="updateAppendEquipmentSearchResult"
                                    class="no-border single-line">
                                    {{ getLangText('global/search') }}
                                </cy-button>
                            </cy-title-input>
                            <transition-group v-if="appendEquipmentWindowState.searchResult.length != 0" name="flip-list"
                                class="search-result" tag="div" mode="out-in">
                                <cy-button v-for="(data, i) in appendEquipmentWindowState.searchResult"
                                    type="description" :iconify-name="data.categoryIcon"
                                    :key="data.origin.id" @click="appendEquipmentSearchResultSelect(data, i)">
                                    {{ data.origin.name }}
                                    <template v-slot:description>
                            ...
                            <div class="search-result-selected bottom-menu">
                                <div class="title">
                                    <cy-button type="line" class="no-border" iconify-name="mdi-checkbox-multiple-blank-circle"
                                        @click="appendEquipmentWindowState.searchResultSelectedVisible = !appendEquipmentWindowState.searchResultSelectedVisible">
                                ...
                                <transition name="fade">
                                    <transition-group v-show="appendEquipmentWindowState.searchResultSelectedVisible" name="flip-list" class="content" tag="div">
                                        <cy-button v-for="(data, i) in appendEquipmentWindowState.searchResultSelected"
                                            type="line" class="inline"
                                            :iconify-name="data.categoryIcon"
                                            @click="appendEquipmentSearchResultSelectedRemove(i)"
                                            :key="data.origin.id">
                                            {{ data.origin.name }}
                                            <template v-slot:content-right>
                                ...
                                <div class="tail-buttons" v-show="appendEquipmentSelectedNumber > 0">
                                    <cy-button class="no-border" iconify-name="ic-round-done"
                                        @click="handleSelectedEquipments('append')">
                                        {{ getLangText('global/confirm') }}
                                    </cy-button>
                                    <cy-button class="no-border" iconify-name="ic-round-clear"
                                        @click="handleSelectedEquipments('clear')">
                                        {{ getLangText('global/clear') }}
                                    </cy-button>
                                </div>
                            </div>
                        </div>
                        <div v-else-if="appendEquipmentWindowState.action == 'custom'" key="custom">
                        </div>
                    </transition>
                </template>
            </cy-window>
            <cy-window :visible="selectCrystalWindowState.visible"
                @close-window="selectCrystalWindowState.visible = false">
                <template v-slot:title>{{ langText('select crystal/title') }}</template>
                <template v-slot:default>
                    <cy-button v-for="(category, i) in crystalSearchResult"
                        :key="category.id"
                        iconify-name="bx-bx-cube-alt" type="drop-down">
                        {{ langText('select crystal/category title')[category.id] }}
                        <template v-slot:menu>
                            <cy-button type="line" class="no-border"
                                v-for="(c, i) in category.crystals"
                                iconify-name="bx-bx-cube-alt" :key="c.id"
                    ...
                    <div class="bottom-menu" v-show="selectCrystalWindowState.currentCrystals.length > 0">
                        <div>
                            <cy-button type="line" iconify-name="bx-bx-cube-alt"
                            v-for="(c, i) in selectCrystalWindowState.currentCrystals"
                            @click="removeSelectedCrystal(i)"
                            :key="c.id">
                            {{ c.name }}
                            <template v-slot:content-right>
                                <cy-icon-text iconify-name="ic-round-close" />
    ...
    export default {
        ...
        computed: {
                ...
                return state.crystalCategorys
                    ...
                    .map(c => {
                        return {
                            id: c.id,
                            crystals: c.crystals
                                .filter(p => !exclude_ids.includes(p.id))
                        };
                    });
            },
            ...
            appendEquipmentSelectedNumber(){
                return this.appendEquipmentWindowState.searchResultSelected.length;
            },
        ...
        methods: {
            ...
            handleSelectedEquipments(action){
                ...
                if ( action == 'append' ){
                    this.equipments.push(...state.searchResultSelected.map(p => p.origin));
                    ShowMessage(this.langText('Warn/append equipments successfully', [this.appendEquipmentSelectedNumber]));
                    state.searchResultSelected = [];
                    this.appendEquipmentWindowState.visible = false;
                }
                else {
                    state.searchResultSelected = [];
                    ShowMessage(this.langText('Warn/clear equipments completed'));
                    this.updateAppendEquipmentSearchResult();
                }
            },
            appendEquipmentSearchResultSelectedRemove(index){
                const state = this.appendEquipmentWindowState;
                state.searchResultSelected.splice(index, 1);
                this.updateAppendEquipmentSearchResult();
            },
            appendEquipmentSearchResultSelect(data, index){
                const state = this.appendEquipmentWindowState;
                state.searchResult.splice(index, 1);
                state.searchResultSelected.push(data);
            },
            ...
            updateAppendEquipmentSearchResult(){
                const text = this.$refs.appendEquipmentSearchText.value;
                ...
                const state = this.appendEquipmentWindowState;
                state.searchResult = t
                    .filter(item => !this.equipments.find(eq => eq.id == item.id))
                    .filter(item => !state.searchResultSelected.find(data => data.origin.id == item.id) )
                    .map(item => {
                        const o = this.itemEquipmentConvert(item);
