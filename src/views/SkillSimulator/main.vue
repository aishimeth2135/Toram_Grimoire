        ...
        <transition name="fade">
          <div class="inner-menu select-skill-tree" v-show="selectSkillTreeVisible">
            <template v-for="(stc, index) in currentSkillRootState.skillTreeCategoryStates">
              ...
              <div class="content">
                <cy-button v-for="(st, index2) in stc.skillTreeStates"
                  type="line" class="icon-small selection"
                  icon-id="rabbit-book" :key="stc.origin.id + '-' + st.origin.id"
          ...
          <div class="inner-menu" v-show="jumpSkillTreeVisible">
            ...
            <template v-else>
              <div class="top Cyteria line-space-bottom">
                <cy-button class="no-border no-padding"
                  iconify-name="ic:round-details"
              ...
              <div v-show="jumpSkillTreeShowDetail && currentStarGemList.length != 0" class="content">
                <cy-button v-for="(o, i) in currentStarGemList"
                  type="line" class="icon-small"
                  :key="o.skillTreeState.origin.id"
                  @click="jumpToSkillTree(o.skillTreeState.origin)"
                  iconify-name="mdi:judaism">
          ...
          <div class="inner-menu build-information" v-show="buildInformationVisible">
            <cy-title-input iconify-name="ant-design:build-outlined">
              ...
              <cy-button iconify-name="bx-bx-search-alt-2" type="icon-only"
                :iconify-name="selectBuildVisible ? 'ic-round-keyboard-arrow-up' : 'ic-round-keyboard-arrow-down'"
                @click="selectBuildVisible = !selectBuildVisible" />
            </cy-title-input>
            <transition name="fade">
              <div class="select-build" v-if="selectBuildVisible">
                <transition-group name="fade" tag="div">
                  <cy-button v-for="(state, i) in skillRootStates"
                    type="line" class="selection"
                    iconify-name="ant-design:build-outlined"
                    :key="state.stateId"
            ...
            <div class="buttons-content">
              <cy-button type="line" @click="copyCurrentBuild"
                iconify-name="mdi:content-copy">
                {{ getLangText('global/copy') }}
              </cy-button>
              <cy-button type="line" @click="deleteCurrentBuild"
                iconify-name="ic:round-delete-outline">
                {{ getLangText('global/delete') }}
              </cy-button>
              <cy-button type="line" @click="exportCurrentBuildText"
                iconify-name="ic-round-text-fields">
                {{ langText('export text') }}
              </cy-button>
              <cy-button type="line" @click="exportCurrentBuildImage"
                iconify-name="mdi:content-copy">
...
<script>
  ...
  import {getSkillElementId} from "@lib/SkillSystem/module/SkillElementMethods.js";
