import Iconify from '@iconify/iconify'

/**
 * Get custom icon by icon ID.
 */
export default function Icons(name: string): string {
  switch (name) {
    case 'potum':
      return '<svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="1250pt" height="1250pt" viewBox="0 0 1250 1250" preserveAspectRatio="xMidYMid meet">' +
          '<g transform="translate(0,1250) scale(0.1,-0.1)" fill="currentcolor" stroke="none">' +
          '<path d="M1468 11473 c-29 -34 -57 -98 -75 -173 -15 -62 -18 -111 -16 -277 l2 -201 -102 34 c-112 38 -148 42 -168 18 -15 -18 -20 -114 -8 -164 4 -19 15 -100 24 -180 56 -495 128 -757 306 -1113 76 -153 214 -396 234 -412 7 -6 78 -141 158 -300 364 -728 566 -1026 995 -1466 l163 -168 -50 -203 c-137 -556 -160 -746 -168 -1370 l-5 -428 -55 -82 c-70 -108 -155 -259 -203 -361 -96 -210 -132 -392 -146 -743 l-7 -159 27 -32 c67 -80 199 -105 317 -59 37 14 69 22 73 19 3 -3 8 2 12 11 3 9 15 16 25 16 10 0 62 11 115 25 102 27 198 71 252 118 l34 28 62 -133 c35 -73 79 -160 99 -193 l35 -60 -221 -6 c-231 -6 -245 -8 -492 -40 -169 -23 -172 -23 -310 -44 -354 -53 -385 -59 -435 -85 -45 -23 -42 -7 -42 -205 0 -139 -13 -216 -66 -397 -33 -112 -30 -132 28 -159 33 -16 55 -15 368 12 389 32 865 65 1177 79 264 13 1155 6 1345 -9 74 -6 218 -18 320 -26 221 -19 532 -53 920 -101 157 -19 292 -36 300 -36 8 0 107 9 220 21 113 11 230 23 260 26 30 3 150 14 265 24 950 88 1527 101 2285 51 204 -14 768 -65 993 -91 150 -17 181 -18 205 -7 30 15 28 -14 15 253 -5 103 -2 140 21 264 24 132 25 146 11 167 -8 13 -24 24 -34 24 -10 0 -72 13 -137 29 -467 115 -798 166 -1299 201 -88 7 -183 15 -211 18 l-52 7 80 95 c111 131 206 278 295 455 42 82 77 152 79 154 2 2 31 -19 66 -46 152 -120 255 -176 398 -218 52 -15 163 -61 245 -100 173 -84 230 -105 286 -105 73 0 119 41 148 130 10 32 6 47 -43 175 -168 435 -316 713 -586 1103 -127 183 -128 184 -134 332 -2 63 -7 122 -9 130 -3 8 -11 94 -17 190 -10 158 -18 263 -36 445 -3 33 -12 112 -20 175 -9 63 -18 144 -21 180 -3 36 -9 70 -13 75 -4 6 -13 39 -20 75 -16 81 -73 261 -114 360 -30 72 -31 76 -16 107 8 18 50 68 93 111 42 43 77 86 77 94 0 9 25 41 55 72 30 30 55 59 55 63 1 15 196 243 397 465 155 171 503 524 658 669 154 143 526 469 739 649 293 248 381 320 387 320 10 0 119 180 119 197 0 8 5 23 10 34 23 42 -16 69 -124 89 -33 5 -61 12 -63 13 -2 2 17 22 43 43 72 61 142 134 147 154 7 27 -18 56 -51 63 -52 10 -661 67 -822 76 -219 13 -695 5 -870 -14 -571 -62 -1039 -208 -1763 -551 -295 -140 -1053 -539 -1090 -573 -16 -16 -24 -16 -80 -5 -399 82 -658 113 -957 114 -287 2 -542 -22 -1110 -104 l-235 -34 -110 70 c-194 125 -241 156 -658 427 -423 275 -994 659 -1350 908 -230 161 -773 557 -1107 808 -118 89 -225 167 -237 173 -31 17 -68 15 -85 -5z"/>' +
          '</g></svg>'
    case 'rabbit-book':
      return '<svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24"><g fill="none" stroke="currentcolor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></g><g transform="scale(0.58) translate(8, 4)"><path d="M18.05 21l-2.73-4.74c0-1.73-1.07-2.84-2.37-2.84c-.9 0-1.68.5-2.08 1.24c.33-.19.72-.29 1.13-.29c1.3 0 2.36 1.06 2.36 2.36c0 1.31-1.05 2.38-2.36 2.38h3.3V21H6.79c-.24 0-.49-.09-.67-.28a.948.948 0 0 1 0-1.34l.5-.5c-.34-.15-.62-.38-.9-.62c-.22.5-.72.85-1.3.85a1.425 1.425 0 0 1 0-2.85l.47.08v-1.97a4.73 4.73 0 0 1 4.74-4.74h.02c2.12.01 3.77.84 3.77-.47c0-.93.2-1.3.54-1.82c-.73-.34-1.56-.55-2.43-.55c-.53 0-.95-.42-.95-.95c0-.43.28-.79.67-.91l-.67-.04c-.52 0-.95-.42-.95-.94c0-.53.43-.95.95-.95h.95c2.1 0 3.94 1.15 4.93 2.85l.28-.01c.71 0 1.37.23 1.91.61l.45.38c2.17 1.95 1.9 3.27 1.9 3.28c0 1.28-1.06 2.33-2.35 2.33l-.49-.05v.08c0 1.11-.48 2.1-1.23 2.8L20.24 21h-2.19m.11-13.26c-.53 0-.95.42-.95.94c0 .53.42.95.95.95c.52 0 .95-.42.95-.95c0-.52-.43-.94-.95-.94z" fill="currentcolor"/></g></svg>'
    case 'grimoire-cat': {
      const mainColor = 'var(--app-favicon-color-main)'
      const subColor = 'var(--app-favicon-color-sub)'
      return '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 81.91 95.07">' +
        '<path ' +
          'd="M67.44,81.51,10.72,92.17c-.76.17-1.37-.36-1.34-.78V15.82c-.06-1.13.61-1.43,1.37-1.59L67.44,2.83A1.11,1.11,0,0,1,68.8,4V79.75A1.85,1.85,0,0,1,67.44,81.51Z" ' +
          'transform="translate(-9.35 -2.8)" ' +
          `fill="${mainColor}"` +
        '/>' +
        '<polygon points="66.58 4.8 9.45 16.3 0.56 13.11 57.51 1.83 66.58 4.8" fill="#fff"/>' +
        `<polygon points="66.85 83.55 10.38 93.94 9.45 16.3 66.58 4.8 66.85 83.55" fill="${subColor}"/>` +
        '<path ' +
          'd="M19.19,96.74,11.3,93.11a2.67,2.67,0,0,1-2-2.11l0-75.18c0-.82.88-1.19,1.95-.83l8.4,3.11c1.07.36,1.4.86,1.4,1.67V95.91a.84.84,0,0,1-.18.56,1.3,1.3,0,0,1-1.22.39A2.62,2.62,0,0,1,19.19,96.74Z" ' +
          'transform="translate(-9.35 -2.8)" ' +
          `fill="${mainColor}"` +
        '/>' +
        '<path ' +
          'd="M89.74,93.23,23.87,97.87a1.37,1.37,0,0,1-1.52-1.35V20.68a1.63,1.63,0,0,1,1.52-1.57l65.87-4.64a1.37,1.37,0,0,1,1.52,1.36V91.66A1.63,1.63,0,0,1,89.74,93.23Z" transform="translate(-9.35 -2.8)" ' +
          `fill="${mainColor}"` +
        '/>' +
        '<path ' +
          'd="M84.31,86l-55,3.92A1.15,1.15,0,0,1,28,88.72V24.38a1.37,1.37,0,0,1,1.27-1.33l55-3.93a1.15,1.15,0,0,1,1.27,1.15V84.62A1.37,1.37,0,0,1,84.31,86Z" ' +
          'transform="translate(-9.35 -2.8)" ' +
          'fill="#fff"' +
        '/>' +
        '<path ' +
          'd="M71.23,52.8a55.26,55.26,0,0,0,.25-8.17c-.22-2.41-4.43-.59-7.83.67a15.22,15.22,0,0,0-5-.45,14.2,14.2,0,0,0-5.38,1.28c-3.68-.92-8.31-2.26-8.42.3A54.19,54.19,0,0,0,45.51,55a8.43,8.43,0,0,0-.84,3.84c.15,5.9,6.77,9.94,14.52,9.07,7.46-.84,13.2-6,13.07-11.49A9,9,0,0,0,71.23,52.8Z" ' +
          'transform="translate(-9.35 -2.8)" ' +
          `fill="${mainColor}"` +
        '/>' +
      '</svg>'
    }
  }
  return ''
}


function loadIconifyData(name: string) {
  return new Promise((resolve) => {
    if (Iconify.iconExists(name)) {
      resolve(Iconify.getIcon(name))
      return
    }
    Iconify.loadIcons([name], () => {
      resolve(Iconify.getIcon(name))
    })
  })
}

export { loadIconifyData }
