...
export function setupCalculationExpectedResult(calculation: Ref<Calculation>, calculationOptions?: Ref<CalcResultOptions>) {
  ...
  const getStabilityExpected = () => calculation.value.containers.get(CalculationContainerIds.Stability)!.result()
  ...
  const baseResultCriticalRate = computed(() => {
    ...
    return (getStabilityExpected() * ac + (grazeStability + 100) / 2 * (100 - ac)) * cr / 1000000
  })
  ...
  const baseResultWithoutCriticalRate = computed(() => {
    ...
    const grazeStability = Math.floor(getStability() / 2)
    return (getStabilityExpected() * ac + (grazeStability + 100) / 2 * Math.max(0, pac - ac)) * (100 - cr) / 1000000
  })

  const expectedResult = computed(() => {
    const expectedResultCritical = baseResultCritical.value * baseResultCriticalRate.value
    const expectedResultWithoutCritical = baseResultWithoutCritical.value * baseResultWithoutCriticalRate.value
